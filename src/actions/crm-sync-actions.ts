...
import { syncLeadToBrevo, type SyncLeadToBrevoInput, type SyncLeadToBrevoOutput } from '@/ai/flows/sync-lead-to-brevo-flow';
import { syncLeadToHubspot, type SyncLeadToHubspotInput, type SyncLeadToHubspotOutput } from '@/ai/flows/sync-lead-to-hubspot-flow';
...
// It NO LONGER updates local storage.
...
export async function syncSelectedLeadsAction(leadIds: string[], companyId: string, brevoApiKey?: string, hubspotApiKey?: string): Promise<BulkSyncResult> {
  ...
  // Group promises by lead to handle cases where both platforms are synced
  const syncsByLead: Record<string, Promise<SyncDetail>[]> = {};

  if (brevoApiKey) {
    leadsToSync.forEach(lead => {
      if (!syncsByLead[lead.id]) syncsByLead[lead.id] = [];
      syncsByLead[lead.id].push(syncSingleLeadToBrevoAction(lead, brevoApiKey));
    });
  }
  if (hubspotApiKey) {
    leadsToSync.forEach(lead => {
      if (!syncsByLead[lead.id]) syncsByLead[lead.id] = [];
      syncsByLead[lead.id].push(syncSingleLeadToHubspotAction(lead, hubspotApiKey));
    });
  }

  const allSyncPromises = Object.values(syncsByLead).flat();

  if (allSyncPromises.length === 0) {
    // This case happens if keys are missing. The UI should prevent this, but it's a safe fallback.
  ...
  const results = await Promise.all(allSyncPromises);
