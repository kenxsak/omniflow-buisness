...
import { serverDb as db } from './firebase-server';
import { 
  collection, 
  doc, 
  addDoc, 
...
export async function createServerAICampaignDraft(
  input: CreateAICampaignDraftInput
): Promise<UnifiedCampaignDraft | null> {
  if (!db) {
    console.error('Server Firebase not initialized');
    return null;
  }
  ...
  try {
    ...
    console.error('Error creating server AI campaign draft:', error);
    return null;
  }
}

export async function updateServerAICampaignDraft(
  ...
  try {
    ...
    console.error('Error updating server AI campaign draft:', error);
    return false;
  }
}

export async function getServerAICampaignDraft(
  companyId: string,
  draftId: string
): Promise<UnifiedCampaignDraft | null> {
  if (!db) {
  ...
  try {
    ...
    console.error('Error getting server AI campaign draft:', error);
    return null;
  }
}

export async function getServerAICampaignDrafts(
  companyId: string
  ...
  try {
    ...
    console.error('Error getting server AI campaign drafts:', error);
    return [];
  }
}

export async function deleteServerAICampaignDraft(
  companyId: string,
  draftId: string
): Promise<boolean> {
  if (!db) {
  ...
  try {
    ...
    console.error('Error deleting server AI campaign draft:', error);
    return false;
  }
}
